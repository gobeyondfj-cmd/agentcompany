...
from agentcompany.core.role import Role
from agentcompany.core.task import Task
from agentcompany.core.message_bus import MessageBus
from agentcompany.core.cost_tracker import CostTracker
from agentcompany.llm.base import LLMMessage, BaseLLMProvider, ToolDefinition
from agentcompany.tools.registry import ToolRegistry

if TYPE_CHECKING:
    from agentcompany.storage.database import Database

logger = logging.getLogger("agentcompany.agent")
...
class Agent:
    ...
    async def think(self, task: Task) -> str:
        ...
        if self.provider is None:
            task.fail("LLM provider not configured. Set an API key in .agentcompany/config.yaml")
    ...
    async def chat(self, message: str) -> str:
        ...
        if self.provider is None:
            return "Error: LLM provider not configured. Set an API key in .agentcompany/config.yaml"
